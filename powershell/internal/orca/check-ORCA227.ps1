        ...
        ForEach ($AcceptedDomain in $Config["AcceptedDomains"]) {
            ...
            ForEach ($Rule in ($Config["SafeAttachmentsRules"] | Sort-Object Priority)) {
                if ($Rule.State -eq "Enabled") {
                    ...
                    # If RecipientDomainIs is empty/null, it means "All recipients" - applies to all domains
                    $AppliesToDomain = $false
                    if ($null -eq $Rule.RecipientDomainIs -or $Rule.RecipientDomainIs.Count -eq 0) {
                        # "All recipients" - applies to all domains unless excluded
                        $AppliesToDomain = $true
                    } elseif ($Rule.RecipientDomainIs -contains $AcceptedDomain.Name) {
                        # Explicit domain match
                        $AppliesToDomain = $true
                    }

                    if ($AppliesToDomain -and ($Rule.ExceptIfRecipientDomainIs -notcontains $AcceptedDomain.Name) -and ($null -eq $Rule.ExceptIfSentToMemberOf ) -and ($null -eq $Rule.ExceptIfSentTo) ) {
            ...
            # Check EOPProtectionPolicyRule for Safe Attachments policies (Standard/Strict preset security policies for EOP)
