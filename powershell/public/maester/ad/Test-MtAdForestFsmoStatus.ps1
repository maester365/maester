<#
.SYNOPSIS
    Checks AD Forest-level FSMO roles

.DESCRIPTION
    Identifies if forest-level FSMO roles are on the same domain controller
...
.EXAMPLE
    Test-MtAdForestFsmoStatus

    Returns true if AD Forest-level FSMO roles are on the same DC

.LINK
    https://maester.dev/docs/commands/Test-MtAdForestFsmoStatus
#>
function Test-MtAdForestFsmoStatus {
    [CmdletBinding()]
    ...
    #region Collect
    $AdObjects.Data.DomainNamingMaster = $AdObjects.Forest.DomainNamingMaster
    $AdObjects.Data.SchemaMaster = $AdObjects.Forest.SchemaMaster
    ...
    $Tests = @{
        CommonFsmo = @{
            Name        = "Forest FSMO roles are on common domain controller"
            Value       = $AdObjects.Data.CommonFsmo
            Threshold   = $true
            Indicator   = "="
            Description = "Validates the Domain Naming Master and Schema Master FSMO roles are on the same domain controller"
            Status      = $null
        }
    }
    #endregion
