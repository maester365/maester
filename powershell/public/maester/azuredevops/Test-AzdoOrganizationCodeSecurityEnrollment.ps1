...
.DESCRIPTION
    ...
    auto enablement setting turned on while every existing repository remains unscanned. This test assesses
...
.LINK
    https://maester.dev/docs/commands/Test-AzdoOrganizationCodeSecurityEnrollment
#>
function Test-AzdoOrganizationCodeSecurityEnrollment {
    [CmdletBinding()]
    [OutputType([bool])]
    param()

    Write-Verbose "Running Test-AzdoOrganizationCodeSecurityEnrollment"
    ...
    if ($Repositories.Count -eq 0) {
        $Message = "The Advanced Security org enablement API did not return any repositories, so Code Security coverage cannot be assessed."
        Write-Verbose $Message
        Add-MtTestResultDetail -SkippedBecause Custom -SkippedCustomReason $Message
        return $null
    }
    ...
    if ($Unenrolled.Count -gt 0) {
        ...
        $resultMarkdown += "`nRepositories not enrolled in Code Security, by project:`n`n"
