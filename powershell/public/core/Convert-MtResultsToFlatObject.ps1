    ...
    process {
        ...
        if ($PSBoundParameters.ContainsKey('CsvFilePath')) {
            if ($CsvFilePath -match '\.xlsx$') {
                Write-Warning 'You are exporting a CSV file, but the file path ends with the .XLSX extension. Please rename with the proper extension or use the '-ExcelFilePath' parameter if you want an Excel file.' -WarningAction Continue
            }
        ...
        if ($PSBoundParameters.ContainsKey('ExcelFilePath')) {
            if ($ExcelFilePath -match '\.csv$') {
                Write-Warning 'You are exporting an Excel file, but the file path ends with the .CSV extension. Please rename with the proper extension or use the '-CsvFilePath' parameter if you want a CSV file.' -WarningAction Continue
            }
