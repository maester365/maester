...
If `GitHubAllowMemberIssueDeletion` is set to literal boolean `true` in `maester-config.json`, a true value is reported as requiring manual review instead of a hard failure. That opt-in is intended for organizations that have completed and documented the CIS Branch A trusted repository-admin review.
