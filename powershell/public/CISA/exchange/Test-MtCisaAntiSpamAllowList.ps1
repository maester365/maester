...
Function Test-MtCisaAntiSpamAllowList {
    ...
    $resultPolicy = $policy | Where-Object {`
        ($_.IPAllowList|Measure-Object).Count -eq 0 -and `
    ...
    if ($testResult) {
        $testResultMarkdown = "Well done. Your tenant does not allow IPs for Safe List.`n`n%TestResult%"
        $result = "[✅ Pass]($portalLink)"
    } else {
        $testResultMarkdown = "Your tenant does allow IPs for Safe List.`n`n%TestResult%"
