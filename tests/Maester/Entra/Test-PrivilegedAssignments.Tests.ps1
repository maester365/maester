...
Describe "Privileged Identity Management (PIM) - Alerts" -Tag "Privileged", "Security", "All" -Skip:( $EntraIDPlan -ne "P2" ) {
    ...
    It "MT.1030: Eligible role assignments on Control Plane are in use by administrators. See https://maester.dev/docs/tests/MT.1030" -Skip:( $EntraIDPlan -ne "P2" ) -Tag "MT.1030" {
        $Check = Test-MtPimAlertsExists -AlertId "RedundantAssignmentAlert" -FilteredAccessLevel "ControlPlane"
        $check.numberOfAffectedItems -eq "0" | Should -Be $true -Because $check.securityImpact
    }
    It "MT.1031: Privileged role on Control Plane are managed by PIM only. See https://maester.dev/docs/tests/MT.1031" -Skip:( $EntraIDPlan -ne "P2" ) -Tag "MT.1031" {
        $Check = Test-MtPimAlertsExists -AlertId "RolesAssignedOutsidePimAlert" -FilteredAccessLevel "ControlPlane"
        $check.numberOfAffectedItems -eq "0" | Should -Be $true -Because $check.securityImpact
    }
    It "MT.1032: Limited number of Global Admins are assigned. See https://maester.dev/docs/tests/MT.1032" -Skip:( $EntraIDPlan -ne "P2" ) -Tag "MT.1032" {
