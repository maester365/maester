...
Describe "ORCA" -Tag "ORCA", "ORCA120", "EXO", "Security", "All" {
    It "ORCA120: Zero Hour Autopurge Enabled for Phish" {
        ...
        if(!(Test-MtConnection ExchangeOnline)){
            ...
            $obj = New-Object -TypeName check-ORCA120_phish
            $obj.Run($Collection)
            $result = ($obj.Completed -and $obj.Result -eq "Pass")

            $resultMarkdown = "Zero Hour Autopurge - Zero Hour Autopurge Enabled for Phish - 120-phish`n`n"
            if($result){
