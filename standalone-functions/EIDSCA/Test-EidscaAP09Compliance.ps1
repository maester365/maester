function Test-EidscaAP09Compliance {
    <#
    .SYNOPSIS
    Default Authorization Settings - Allow user consent on risk-based apps

    .DESCRIPTION
    Validates EIDSCA control AP09.
    Returns true if compliant, false if non-compliant, null if skipped or error.

    .EXAMPLE
    $result = Test-EidscaAP09Compliance
    ...
    try {
        ...
        $actualValue = $response.allowUserConsentForRiskyApps
    } catch {
    ...
    if ($testResult) {
        Write-Verbose "EIDSCA AP09: Compliant"
    } else {
        Write-Verbose "EIDSCA AP09: Non-Compliant - Expected: false, Actual: $actualValue"
    }
