function Test-EidscaAF02Compliance {
    <#
    .SYNOPSIS
    Authentication Method - FIDO2 security key - Allow self-service set up

    .DESCRIPTION
    Validates EIDSCA control AF02.
    Returns true if compliant, false if non-compliant, null if skipped or error.

    .EXAMPLE
    $result = Test-EidscaAF02Compliance
    ...
    try {
        ...
        $actualValue = $response.isSelfServiceRegistrationAllowed
    } catch {
    ...
    if ($testResult) {
        Write-Verbose "EIDSCA AF02: Compliant"
    } else {
        Write-Verbose "EIDSCA AF02: Non-Compliant - Expected: true, Actual: $actualValue"
    }
