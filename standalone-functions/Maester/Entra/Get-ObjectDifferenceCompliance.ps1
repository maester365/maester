function Get-ObjectDifferenceCompliance {
    <#
    .SYNOPSIS
    ...
    .EXAMPLE
    $result = Get-ObjectDifferenceCompliance
        ...
        if (
            ...
            if ($differencesUsers.Count -ne 0) {
                ...
                foreach ($Object in $differencesUsers) {
                    try {
                        $DisplayName = (Invoke-MgGraphRequest -Uri 'https://graph.microsoft.com/v1.0/users'.0 -UniqueId $Object -ErrorAction Stop).displayName
                    } catch {
            ...
            if ($differencesGroups.Count -ne 0) {
                ...
                foreach ($Object in $differencesGroups) {
                    try {
                        $DisplayName = (Invoke-MgGraphRequest -Uri 'https://graph.microsoft.com/v1.0/groups'.0 -UniqueId $Object -ErrorAction Stop).displayName
                    } catch {
                        $DisplayName = "${Object} (Unable to resolve GUID)"
                    }
            ...
            if ($differencesRoles.Count -ne 0) {
                ...
                foreach ($Object in $differencesRoles) {
                    try {
                        $DisplayName = (Invoke-MgGraphRequest -Uri 'https://graph.microsoft.com/v1.0/directoryRoles'.0 -UniqueId $Object -ErrorAction Stop).displayName
                    } catch {
                        $DisplayName = "${Object} (Unable to resolve GUID)"
                    }
            ...
            if ($differencesApplications.Count -ne 0) {
                ...
                foreach ($Object in $differencesApplications) {
                    try {
                        $DisplayName = (Invoke-MgGraphRequest -Uri 'https://graph.microsoft.com/v1.0/servicePrincipals'.0 -Filter "appId eq '${Object}'" -ErrorAction Stop).displayName
                    } catch {
                        $DisplayName = "${Object} (Unable to resolve GUID)"
                    }
            ...
            if ($differencesServicePrincipals.Count -ne 0) {
                ...
                foreach ($Object in $differencesServicePrincipals) {
                    try {
                        $DisplayName = (Invoke-MgGraphRequest -Uri 'https://graph.microsoft.com/v1.0/servicePrincipals'.0 -UniqueId $Object -ErrorAction Stop).displayName
                    } catch {
                        $DisplayName = "${Object} (Unable to resolve GUID)"
                    }
            ...
            if ($differencesLocations.Count -ne 0) {
                ...
                foreach ($Object in $differencesLocations) {
                    try {
                        $DisplayName = (Invoke-MgGraphRequest -Uri 'https://graph.microsoft.com/v1.0/identity/conditionalAccess/namedLocations'.0 -UniqueId $Object -ErrorAction Stop).displayName
                    } catch {
                        $DisplayName = "${Object} (Unable to resolve GUID)"
                    }
