function Test-MtEntitlementManagementValidResourceRolesCompliance {
    <#
    .SYNOPSIS
    ...
    .EXAMPLE
    $result = Test-MtEntitlementManagementValidResourceRolesCompliance
    ...
    try {
        ...
        foreach ($catalog in $catalogArray) {
            ...
            try {
                ...
                $resources = Invoke-MgGraphRequest -Uri 'https://graph.microsoft.com/beta/identityGovernance/entitlementManagement/accessPackageCatalogs/$catalogId/accessPackageResources'
                ...
                foreach ($resource in $resourceArray) {
                    ...
                    switch -Wildcard ($resourceType) {
                        ...
                        "*Application*" {
                            ...
                            try {
                                $sp = Invoke-MgGraphRequest -Uri 'https://graph.microsoft.com/beta/servicePrincipals/$resourceOriginId' -ErrorAction Stop
                                ...
                                if (-not $sp -or -not $sp.id) {
                                    ...
                                    # Check for stale app roles using cached packages
                                        ...
                                        foreach ($package in $catalogPackages) {
                                            try {
                                                $roleScopes = Invoke-MgGraphRequest -Uri 'https://graph.microsoft.com/beta/identityGovernance/entitlementManagement/accessPackages/$($package.id)/accessPackageResourceRoleScopes?`$expand=accessPackageResourceRole,accessPackageResourceScope'
                            ...
                            try {
                                if ($resourceOriginId -match "^https://") {
                                    ...
                                    if ($siteUrl -match '^https://([^/]+)(.*)$') {
                                        ...
                                        try {
                                            $site = Invoke-MgGraphRequest -Uri 'https://graph.microsoft.com/v1.0/sites/$siteIdentifier'.0
