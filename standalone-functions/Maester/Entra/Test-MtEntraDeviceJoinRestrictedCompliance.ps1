function Test-MtEntraDeviceJoinRestrictedCompliance {
    <#
    .SYNOPSIS
    ...
    .EXAMPLE
    $result = Test-MtEntraDeviceJoinRestrictedCompliance
    ...
    try {
        ...
        $settings = Invoke-MgGraphRequest -Uri 'https://graph.microsoft.com/v1.0/policies/deviceRegistrationPolicy' -ApiVersion 'beta' -ErrorAction Stop
            ...
            switch ($allowedToJoinType) {
                '#microsoft.graph.enumeratedDeviceRegistrationMembership' {
                    ...
                    foreach ($userId in $allowedUsers) {
                        try {
                            $user = Invoke-MgGraphRequest -Uri 'https://graph.microsoft.com/v1.0/users/$userId' -ApiVersion 'v1.0' -ErrorAction Stop
                    ...
                    foreach ($groupId in $allowedGroups) {
                        try {
                            $group = Invoke-MgGraphRequest -Uri 'https://graph.microsoft.com/v1.0/groups/$groupId' -ApiVersion 'v1.0' -ErrorAction Stop
