...
Connect-Maester [-SendMail] [-SendTeamsMessage] [-Privileged] [-UseDeviceCode] [[-Environment] <String>]
 ...
 [[-UseLegacySpoClientIdFallback]] [[-SharePointAdminUrl] <String>] [[-SharePointCertificateThumbprint] <String>]
...
Required when Service explicitly includes SharePointOnline.
If SharePointOnline is only included by `-Service All` and this value is omitted, Maester skips SharePoint connection with a warning.
...
### -UseLegacySpoClientIdFallback

If specified and `-SharePointClientId` is omitted, uses the Microsoft.Online.SharePoint.PowerShell first-party client ID as a compatibility fallback.
...
Determines how progress information is handled for this command, such as whether progress is displayed, suppressed, or treated according to the specified action preference.
