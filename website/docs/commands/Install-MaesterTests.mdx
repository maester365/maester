---
sidebar_class_name: hidden
description: Installs the latest ready-made Maester tests built by the Maester team.
id: Install-MaesterTests
title: Install-MaesterTests
hide_title: false
hide_table_of_contents: false
custom_edit_url: https://github.com/maester365/maester/blob/main/powershell/public/Install-MaesterTests.ps1
---

## SYNOPSIS

Installs the latest ready-made Maester tests built by the Maester team.

## SYNTAX

```powershell
Install-MaesterTests [[-Path] <String>] [-ProgressAction <ActionPreference>] [<CommonParameters>]
```

## DESCRIPTION

The Maester team maintains a repository of ready made tests that can be used to verify the configuration of your Microsoft 365 tenant.

The tests can be viewed at https://github.com/maester365/maester/tree/main/tests

## EXAMPLES

### EXAMPLE 1

```powershell
Install-MaesterTests
```

Install the latest set of Maester tests in the current directory.

### EXAMPLE 2

```powershell
Install-MaesterTests -Path .\maester-tests
```

Installs the latest Maester tests in the specified directory.

## PARAMETERS

### -Path

The path to install the Maester tests to, defaults to the current directory.

```yaml
Type: String
Parameter Sets: (All)
Aliases:

Required: False
Position: 1
Default value: .\
Accept pipeline input: False
Accept wildcard characters: False
```

### -ProgressAction

\{\{ Fill ProgressAction Description \}\}

```yaml
Type: ActionPreference
Parameter Sets: (All)
Aliases: proga

Required: False
Position: Named
Default value: None
Accept pipeline input: False
Accept wildcard characters: False
```

### CommonParameters

This cmdlet supports the common parameters: -Debug, -ErrorAction, -ErrorVariable, -InformationAction, -InformationVariable, -OutVariable, -OutBuffer, -PipelineVariable, -Verbose, -WarningAction, and -WarningVariable. For more information, see [about_CommonParameters](http://go.microsoft.com/fwlink/?LinkID=113216).

## INPUTS

## OUTPUTS

## NOTES

## RELATED LINKS
